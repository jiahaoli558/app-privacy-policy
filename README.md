...
為了提供日記記錄、同步、心情分析和地理位置標記等核心功能，本應用程式會收集以下類型的資訊：

*   **您主動提供的內容**:
    *   **日記文字與標題**: 您在應用程式中建立的日記內容。
    *   **照片與影片**: 您從裝置相簿中選取並新增至日記中的媒體檔案。

*   **我們自動收集的資訊**:
    *   **位置資訊**: 經您授權後，我們會在您儲存日記時收集您裝置的精確地理位置，用於在日記中標記地點。
    *   **情緒分析資料**: 我們會對您的日記文字進行本機分析，以產生一個情緒分數，此資料僅與對應的日記綁定，用於提供心情快照功能。

*   **用於資料同步的資訊**:
    *   本應用程式使用蘋果公司提供的 **iCloud (CloudKit)** 服務，在您的不同裝置之間同步您的日記資料（包含文字、媒體檔案和位置資訊）。這意味著您的資料會以加密方式儲存在您的私人 iCloud 帳號中，我們作為開發者無法存取、檢視或分享這些內容。

### 2. 我們如何使用您的資訊

我們僅為以下目的使用您的資訊：

*   **提供核心功能**: 建立、編輯、檢視和管理您的日記。
*   **資料同步**: 透過 iCloud 在您擁有的多個 Apple 裝置間無縫同步您的日記資料。
*   **功能增強**: 提供地理位置標記和心情分析等特色功能。
*   **應用程式維護**: 我們可能會檢視匿名的、彙總的診斷資料（例如應用程式當機報告），以改進應用程式的穩定性和效能。

### 3. 資料的儲存與安全

*   **本機儲存**: 您的日記資料首先會安全地儲存在您裝置本機的 Core Data 資料庫中。
*   **雲端儲存**: 為了同步，您的資料會透過點對點加密的方式儲存在您的私人 iCloud (CloudKit) 帳號中。只有您自己可以透過受信任的裝置存取這些資料。我們無法解密或存取您的任何個人日記內容。
*   **資料安全**: 我們致力於保護您的資料安全。透過利用蘋果的 Core Data 和 CloudKit 框架，我們採用了業界標準的安全措施，以防止資料遺失、濫用、未經授權的存取或揭露。

### 4. 您的權利

由於您的資料儲存在裝置本機和您的私人 iCloud 帳號中，您擁有完全的控制權：

*   **存取與修改**: 您可以隨時在本應用程式中檢視、編輯或刪除您的日記。
*   **停止收集位置**: 您可以隨時透過裝置的系統設定，關閉本應用程式的定位服務授權。
*   **刪除資料**: 當您刪除應用程式時，儲存在裝置本機的資料將被移除。儲存在 iCloud 中的資料，您可以透過管理您的 iCloud 儲存空間來刪除。

### 5. 隱私權政策的變更

我們可能會不時更新本隱私權政策。當發生重大變更時，我们會在應用程式內通知您，或在 App Store 的更新說明中告知。建議您定期查閱以了解最新的隱私權政策。

### 6. 聯絡我們

如果您對本隱私權政策有任何疑問、意見或建議，請透過以下方式與我們聯絡：

*   **開發者**: jiahao li
*   **電子郵件**: gscassli@foxmail.com
